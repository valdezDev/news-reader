...
const GET_ARTICLES_BY_SOURCE = gql`
  query getArticleBySource($ids: [Int!]!, $source: String!) {
...
describe("[Queries.HackerNewsAPI", () => {
  it("fetches an article from the HackerNewsAPI", async () => {
  ...
  it("fetches an array of articles from the HackerNewsAPI", async () => {
    const { server, hackernewsAPI } = constructTestServer();
  ...
  it("fetches an array of all the articles from the HackerNewsAPI", async () => {
    const { server, hackernewsAPI } = constructTestServer();
